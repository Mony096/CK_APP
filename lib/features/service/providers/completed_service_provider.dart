...
//   }
  ...
  Future<dynamic> syncAllOfflineServicesToSAP(BuildContext context) async {
    ...
    for (var servicePayload in completedServices) {
      ...
      try {
        // ðŸ”‘ Decode {ext, data} into temp files only if files exist
        ...
        //         i++;
        //       } catch (err) {
        //         debugPrint("âŒ Error decoding base64 for file $i: $err");
        //       }
        //     } else {
        //       debugPrint("âš ï¸ Skipped invalid entry in fileDataList: $f");
        //     }
        //   }
        // }
        ...
        if (filesToUpload.isNotEmpty) {
          ...
          if (attachmentEntry == null) {
            debugPrint("âš ï¸ Failed to sync attachments for DocEntry: $docEntry");
        ...
        if (response.statusCode == 200) {
          ...
          debugPrint("âœ… Synced DocEntry: $docEntry");
        } else {
          throw Exception(
              "âŒ Failed to sync DocEntry: $docEntry. Status: ${response.statusCode}");
        }
  ...
  // Helper function: convert File â†’ { ext, data }
  ...
  Future<bool> onCompletedServiceOffline({
    ...
    final payload = {
      ...
      "files": fileDataList, // âœ… Each file has {ext, data}
    };
            ...
            children: [
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    Text(
                      "âœ… Service successfully completed in offline mode.",
                      style: TextStyle(
