...
import 'package:bizd_tech_service/features/equipment/presentation/components/general.dart';
import 'package:bizd_tech_service/features/equipment/presentation/components/component.dart';
import 'package:bizd_tech_service/features/equipment/presentation/components/part.dart';
...
class _EquipmentCreateScreenState extends State<EquipmentCreateScreen> {
  ...
  Future<void> _initOffline() async {
    // 1ï¸âƒ£ When creating new equipment
    if (widget.data.isEmpty) {
      installedDate.text = DateFormat('yyyy-MM-dd').format(DateTime.now());
      nextDate.text = DateFormat('yyyy-MM-dd').format(DateTime.now());
      warrantyDate.text = DateFormat('yyyy-MM-dd').format(DateTime.now());
    ...
    // 2ï¸âƒ£ When editing existing equipment
    if (!mounted) return;
    ...
    try {
      ...
      if (offlineData["files"] != null) {
        ...
        for (var fileMap in offlineData["files"]) {
          ...
          // âœ… ensure unique filename (avoids overwriting)
          final filePath =
      ...
      // âœ… Replace images list (not append duplicates)
